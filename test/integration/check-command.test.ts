...
`);
  ...
  // Get ID
  const listResult = await pcheck("--show-ids");
  const idMatch = listResult.stdout.match(/\[x\] Checked task \[([a-f0-9]{8})\]/);
...
`);
  ...
  // Get ID
  const listResult = await pcheck("--show-ids");
  const idMatch = listResult.stdout.match(/\[ \] Unchecked task \[([a-f0-9]{8})\]/);
  const taskId = idMatch?.[1];
...
`);
  ...
  const result = await $`deno run --allow-read --allow-write --allow-run ${join(Deno.cwd(), "src/cli.ts")} check invalid123`
    .cwd(testDir)
    .quiet()
    .noThrow();
...
`);
  ...
  // Get nested task ID
  const listResult = await pcheck("--show-ids");
  const idMatch = listResult.stdout.match(/\[ \] Nested task \[([a-f0-9]{8})\]/);
