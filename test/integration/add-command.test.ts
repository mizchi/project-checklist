...
async function pcheck(...args: string[]) {
  const result = await $`deno run --allow-read --allow-write ${join(Deno.cwd(), "src/cli.ts")} ${args}`
    .cwd(testDir)
    .quiet();
  return {
...
Deno.test("add command - invalid priority format", async () => {
  const result = await $`deno run --allow-read --allow-write ${join(Deno.cwd(), "src/cli.ts")} add -m "Bad priority" -p "5high"`
    .cwd(testDir)
    .quiet()
    .noThrow();
