...
Deno.test("Regression: Bug priorities (P0-P3) should be accepted", async () => {
  ...
  try {
    ...
    assertEquals(
      success,
      true,
      `Command failed: ${new TextDecoder().decode(stderr)}`,
    );
...
`,
    ...
    assertEquals(
      completedSection.includes("- [x] Completed priority task"),
      true,
    );
  } finally {
    await Deno.remove(testDir, { recursive: true });
...
`,
    ...
    assertEquals(
      vacuumedOutput.includes("[P0] Fix critical security bug"),
      true,
    );
    ...
    assertEquals(
      remainingContent.includes("[P0] Fix critical security bug"),
      false,
    );
...
Deno.test("Regression: Config file fileExtensions should filter code TODOs", async () => {
  ...
  try {
    ...
    await Deno.writeTextFile(
      join(testDir, "pcheck.config.json"),
      JSON.stringify({
        code: {
          enabled: true,
          fileExtensions: ["js"],
