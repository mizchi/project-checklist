...
export function validateConfig(config: unknown): ConfigValidationResult {
  ...
  if ("code" in config && config.code !== undefined) {
    if (!isObject(config.code)) {
      ...
      if (
        "enabled" in code && code.enabled !== undefined &&
        !isBoolean(code.enabled)
      ) {
      ...
      if (
        "includeTests" in code && code.includeTests !== undefined &&
        !isBoolean(code.includeTests)
      ) {
        errors.push({
          path: "code.includeTests",
    ...
    if (!isObject(config.output)) {
      ...
      if ("format" in output && output.format !== undefined) {
        if (!["tree", "flat", "json"].includes(output.format as string)) {
          errors.push({
            path: "output.format",
            message: "Must be 'tree', 'flat', or 'json'",
          });
        }
      }
      if (
        "colors" in output && output.colors !== undefined &&
        !isBoolean(output.colors)
      ) {
        errors.push({ path: "output.colors", message: "Must be a boolean" });
      }
      if (
        "quiet" in output && output.quiet !== undefined &&
        !isBoolean(output.quiet)
      ) {
  ...
  if ("search" in config && config.search !== undefined) {
    if (!isObject(config.search)) {
      ...
      if ("engine" in search && search.engine !== undefined) {
        if (
          !["auto", "rg", "git-grep", "grep", "native"].includes(
            search.engine as string,
          )
        ) {
          errors.push({
            path: "search.engine",
            message: "Must be 'auto', 'rg', 'git-grep', 'grep', or 'native'",
          });
        }
      }
    }
