...
function extractTestName(text: string): string | null {
  ...
  const argsMatch = text.match(
    /(?:Deno\.test|it|test|describe)(?:\.skip)?\s*\(([^)]*)\)/s,
  );
...
export async function detectTestCasesWithAst(
  ...
  if (!await checkAstGrepInstalled()) {
    throw new Error(
      "ast-grep is not installed. Please install it first: https://ast-grep.github.io/guide/quick-start.html",
    );
  }
  ...
  const patterns = [
    {
      pattern: AST_GREP_PATTERNS.denoTest,
      type: "test" as const,
      skipped: false,
    },
    {
      pattern: AST_GREP_PATTERNS.denoTestSkip,
      type: "test" as const,
      skipped: true,
    },
    { pattern: AST_GREP_PATTERNS.itTest, type: "it" as const, skipped: false },
    {
      pattern: AST_GREP_PATTERNS.itTestSkip,
      type: "it" as const,
      skipped: true,
    },
    {
      pattern: AST_GREP_PATTERNS.testFn,
      type: "test" as const,
      skipped: false,
    },
    {
      pattern: AST_GREP_PATTERNS.testSkip,
      type: "test" as const,
      skipped: true,
    },
    {
      pattern: AST_GREP_PATTERNS.describe,
      type: "describe" as const,
      skipped: false,
    },
    {
      pattern: AST_GREP_PATTERNS.describeSkip,
      type: "describe" as const,
      skipped: true,
    },
  ];
...
export async function convertAstTestCasesToTodos(
