...
export async function runInteractiveMode(todos: TodoItem[], baseDir: string) {
  ...
  while (true) {
    ...
    const selected = await $.maybeSelect({
    ...
    if (typeof selected === "string" && (selected.startsWith("──") || selected === "─────────────────────────")) {
      continue;
    }
