...
export function getNextConfirmResponse(
  autoResponse: AutoResponse,
): boolean | undefined {
  if (
    !autoResponse.confirmResponses || autoResponse.confirmResponses.length === 0
  ) {
    return undefined;
  }
...
export function getNextMultiSelectResponse(
  autoResponse: AutoResponse,
): number[] | undefined {
  if (
    !autoResponse.multiSelectResponses ||
    autoResponse.multiSelectResponses.length === 0
  ) {
    return undefined;
  }
  ...
  const response =
    autoResponse.multiSelectResponses[autoResponse._multiSelectIndex];
...
export function getNextPromptResponse(
  autoResponse: AutoResponse,
): string | undefined {
  if (
    !autoResponse.promptResponses || autoResponse.promptResponses.length === 0
  ) {
    return undefined;
  }
